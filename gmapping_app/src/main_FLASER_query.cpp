...
{
    g_ack_syn = ack->data;
    cout<<"FLASER_query.cpp: get ack !"<<endl;
}

void savePathCallback(const std_msgs::BoolPtr& ack)
{
    g_save_path_syn = ack->data; 
    cout<<"FLASER_query.cpp: save Path succeed, exist!"<<endl;
}
...
int main(int argc, char**argv)
{
	if(argc<2)
	{
		printf("Should Use As ./main FLASER.log");
		return 0;
	}

	ros::init(argc, argv, "main_FLASER_query");
	ros::NodeHandle n;
    ...
    vector<vector<float> > gtAll;
    vector<vector<float> > odoSaveAll; // dump to file
	...
	//use defined param
    int sick_validPoints = 181; //361;//541.;
	...
	while(inf.good())
	{
		...
		int beam_num = atoi(strtok(NULL, delim.c_str()));
		vector<float> sick;
        sick.resize(beam_num);
		for(int i=0;i<beam_num;i++)
		{
			sick[i] = (atof(strtok(NULL, delim.c_str())));
		}
        if(beam_num < sick_validPoints)
        {
            if(beam_num == sick_allPoints - 1)
            {
                sick.push_back(0);
            }else
            {
                cout<<"wrong sick beam number: "<<beam_num<<" required at least: "<<sick_validPoints-1<<endl;
                break;
            }
        }
        ...
        */
        ...
        // odo
        float theta;
		vector<float> odo, odoSave;
		odo.push_back(atof(strtok(NULL, delim.c_str())));
		odo.push_back(atof(strtok(NULL, delim.c_str())));
		odo.push_back(0.);
		theta = atof(strtok(NULL, delim.c_str()));// * M_PI/180. ;
        odoSave = odo; 
        odoSave[2] = theta;
		...
		// gt
		vector<float> odo_gt;
		odo_gt.push_back(atof(strtok(NULL, delim.c_str())));
		odo_gt.push_back(atof(strtok(NULL, delim.c_str())));
		theta = atof(strtok(NULL, delim.c_str()));// * M_PI/180. ;
		odo_gt.push_back(theta);
        ...
        // float unknown = atof(strtok(NULL, delim.c_str()));
        double t = atof(strtok(NULL, delim.c_str()));   
    
        // add this frame
		odoTime.push_back( t );
		odoAll.push_back(odo);
        gtAll.push_back(odo_gt);
    ...
    if(publish_data_)
    {
        ...
        while(!g_ack_syn)
        {
            cout<<"FLASER_query.cpp: send SYN!"<<endl;
            syn_pub_.publish(ok); 
            ros::spinOnce();
            r2.sleep();
        }
        ...
        while(n.ok() && k<odoAll.size())
        {
            ...
            odoTime[k] = current_time.toSec(); // to match with path
        ...
        {
            cout<<"FLASER.cpp: send save_path SYN!"<<endl;
    ...
    if(save_gt_)
    {
        cout<<"Try to save gt in FLASER_gt.txt"<<endl; 
        ofstream gt("FLASER_gt.txt");
        for(int i=0; i<gtAll.size(); i++)
        {
           gt<<std::fixed<<odoTime[i]<<" "<<gtAll[i][0]<<" "<<gtAll[i][1]<<" "<<gtAll[i][2]<<endl; 
        }
    }
    if(save_odo_)
    {
        cout<<"Try to save odo in FLASER_odo.txt"<<endl; 
        ofstream odo("FLASER_odo.txt"); 
