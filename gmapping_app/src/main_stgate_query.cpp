...
{
    g_ack_syn = ack->data;
    cout<<"fr079_query.cpp: get ack !"<<endl;
}

void savePathCallback(const std_msgs::BoolPtr& ack)
{
    g_save_path_syn = ack->data; 
    cout<<"fr079_query.cpp: save Path succeed, exist!"<<endl;
}
...
int main(int argc, char**argv)
{
	...
	ros::init(argc, argv, "main_stgate_query");
	ros::NodeHandle n;
    ...
    if(publish_data_)
    {
        ...
        while(!g_ack_syn)
        {
            cout<<"fr079_query.cpp: send SYN!"<<endl;
            syn_pub_.publish(ok); 
            ros::spinOnce();
            r2.sleep();
        }
        ...
        {
            cout<<"fr079_query.cpp: send save_path SYN!"<<endl;
    ...
    if(save_gt_)
    {
        cout<<"Try to save odo in stgate_odo.txt"<<endl; 
       //  ofstream gt("stgate_gt.txt");
        ofstream odo("stgate_odo.txt"); 
