...
int main(int argc, char**argv)
{
	...
	ros::init(argc, argv, "main_gmapping_query_lo");
	ros::NodeHandle n;
   ...
   // whether use laser_odo 
    bool use_laser_odo ;
    bool first_scan = true;
    CICPWarpper* pICP = 0;
    OrientedPoint2D last_pose, curr_pose; 
    vector<float> prev_x, prev_y;
    vector<float> curr_x, curr_y; 
    vector<float> cosA, sinA; 
    ...
    private_nh.param("laser_odo", use_laser_odo, false);
    
    if(use_laser_odo)
    {
        pICP = new CICPWarpper;
        pICP->initICP();
        cosA.resize(sick_validPoints);
        sinA.resize(sick_validPoints);
        prev_x.resize(sick_validPoints);
        prev_y.resize(sick_validPoints); 
        curr_x.resize(sick_validPoints);
        curr_y.resize(sick_validPoints);
    }
    ...
    if(publish_data_)
    {
        ...
        while(n.ok() && k<odoAll.size())
        {
            ...
            //first, we'll publish the scan info
            ...
            for(unsigned int i = 0; i < sick_validPoints; ++i)
            {
                ...
                if(use_laser_odo)
                {
                    if(k == 0) // first frame 
                    {
                        double angle = sick_minAngle + i*scan.angle_increment;
                        cosA[i] = cosf(angle);
                        sinA[i] = sinf(angle);
                    }
                    curr_x[i] = scan.ranges[i] * cosA[i];
                    curr_y[i] = scan.ranges[i] * sinA[i];
                }
            }
            scan_pub.publish(scan);

            //second, we'll publish the transform over tf
            tf::Vector3 tr(odoAll[k][0],odoAll[k][1],odoAll[k][2]);
            tf::Quaternion q(odoAll[k][3],odoAll[k][4],odoAll[k][5],odoAll[k][6]);
            if(use_laser_odo) // use laser odo
            {
                if(k == 0) // first scan 
                {
                    double theta = tf::getYaw(q);
                    curr_pose = last_pose = OrientedPoint2D(tr[0], tr[1], theta);
                }else
                {
                    double input[3] = {0}; 
                    double output[3] = {0}; 
                    pICP->ICPMatch(&prev_x[0], &prev_y[0], prev_x.size(),
                                    &curr_x[0], &curr_y[0], curr_x.size(), input, output);
                    if(sqrt(SQ(output[0]) + SQ(output[1])) > 0.5 || fabs(output[2]) > 0.2)
                    {
                        curr_pose = last_pose;
                    }else
                    {
                       curr_pose = last_pose.oplus(OrientedPoint2D(output[0], output[1], output[2]));
                    }
                }
                ROS_WARN("gmapping_app_main_lo.cpp: curr_pose: %f %f %f", curr_pose.x, curr_pose.y, curr_pose.theta);
                q = tf::createQuaternionFromRPY(0,0,curr_pose.theta);               
                tr[0] = curr_pose.x; 
                tr[1] = curr_pose.y;
                prev_x = curr_x; 
                prev_y = curr_y;
