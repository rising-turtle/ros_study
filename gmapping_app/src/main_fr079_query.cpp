...
int main(int argc, char**argv)
{
	if(argc<2)
	{
		printf("Should Use As ./main fr079.log");
		return 0;
	}

	ros::init(argc, argv, "main_fr079_query");
	ros::NodeHandle n;
	...
	while(inf.good())
	{
        ...
        */
        ...
        string magnum = strtok(NULL, delim.c_str());
        if(magnum != string("magnum"))
        {
            cout<<"wrong occured with magnum: "<<magnum<<endl;
            break;
        }
    ...
    if(publish_data_)
    {
        ...
        {
            cout<<"gmapping_query.cpp: send save_path SYN!"<<endl;
    ...
    if(save_gt_)
    {
        cout<<"Try to save groundtruth in fr079_gt.txt, and odo in fr079_odo.txt"<<endl; 
        ofstream gt("fr079_gt.txt");
        ofstream odo("fr079_odo.txt"); 
        assert(gtAll.size() == odoSaveAll.size());
