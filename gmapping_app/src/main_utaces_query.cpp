...
{
    g_ack_syn = ack->data;
    cout<<"utaces_query.cpp: get ack !"<<endl;
}

void savePathCallback(const std_msgs::BoolPtr& ack)
{
    g_save_path_syn = ack->data; 
    cout<<"utaces_query.cpp: save Path succeed, exist!"<<endl;
}
...
int main(int argc, char**argv)
{
	...
	ros::init(argc, argv, "main_utaces_query");
	ros::NodeHandle n;
	...
	//use defined param
	float sick_validPoints = 181; //361;//541.;
    ...
    if(publish_data_)
    {
        ...
        while(!g_ack_syn)
        {
            cout<<"utaces_query.cpp: send SYN!"<<endl;
            syn_pub_.publish(ok); 
            ros::spinOnce();
            r2.sleep();
        }
        ...
        {
            cout<<"utaces_query.cpp: send save_path SYN!"<<endl;
    ...
    if(save_gt_)
    {
        cout<<"Try to save odo in utaces_odo.txt"<<endl; 
       //  ofstream gt("stgate_gt.txt");
        ofstream odo("utaces_odo.txt"); 
