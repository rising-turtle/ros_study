...
if nargin>2
    ...
    FlagDebug = 0; %mostly set to 0
end

%FlagDebug=1; 
%dbstop at 252 in vodometry_dr_ye
...
%disp('vodometry_dr_ye.m:class(img1)')
%class(img1)
%disp('vodometry_dr_ye.m:size(img1)')
%size(img1)
%disp('vodometry_dr_ye.m:max(img1)')
%max(img1)
%disp('vodometry_dr_ye.m:min(img1)')
%min(img1)
...
if myCONFIG.FLAGS.CONFIDENCE_MAP %this is set to 1 in config file
...
RANSAC_STAT.nMatches = size(match,2);
if FlagDebug
    f3=figure(6);
...
if pnum<4
    ...
    for i=1:min(rst,nIterations)
        ...
        %%this_time=toc;
        %%ransac_time=ransac_time+this_time;
        for k=1:cnum
    ...
    %%ransac_time=ransac_time/i;
    %%disp('vodometry_dr_ye:ransac_dr_ye_time:');
    %%disp(ransac_time);
