...
//********************************************************************************************************
ransac_stat publish_sr4000(sr4000handler *sh)
{
	bool fp_exists_last=true, fp_exists_curr=true;		
	ransac_stat rs_obj;	
	sr4000data data_obj_last;
	//cout<<"\npoint_cloud_publisher.cpp: marker 1.1\n";
	sh->getXYZObs(data_obj_last, fp_exists_last); //this function increments the file index internally
	cout<<"point_cloud_publisher.cpp:sh->queue_index_xyz: "<<sh->queue_index_xyz;
	//cout<<"\npoint_cloud_publisher.cpp: marker 1.2\n";
	if (fp_exists_last)
	{
	...
	//cout<<"\npoint_cloud_publisher.cpp: marker 1.5\n";
	int state=1;
	...
	//cout<<"\npoint_cloud_publisher.cpp: marker 1.6\n";
	sh->obs_no++;
	return rs_obj;
}
//********************************************************************************************************
int main(int argc, char** argv)
{
	...
	sr4000handler *sh= new sr4000handler("/home/emaad22/Desktop/SR4k_data");
	cout<<"point_cloud_publisher.cpp:sh->queue_index_xyz: "<<sh->queue_index_xyz;
	ransac_stat rs_obj;
	int limit_obs=20;
	...
	while ((count < limit_obs) && (n.ok()))
	{
		...
		cout<<"\npoint_cloud_publisher.cpp: marker 1\n";
		rs_obj=publish_sr4000(sh);
		cout<<"\npoint_cloud_publisher.cpp: marker 2\n";
		cloud.points.resize(rs_obj.op_pset1.size());

		//we'll also add an intensity channel to the cloud
		cloud.channels.resize(1);
		cloud.channels[0].name = "intensities";
		cloud.channels[0].values.resize(144*176);
		cout<<"\npoint_cloud_publisher.cpp: marker 3\n";
		//copy data to our point cloud
		for(unsigned int j = 0; j < rs_obj.op_pset1.size(); ++j)
		{
			cloud.points[j].x = rs_obj.op_pset1.at(j).x;
			cloud.points[j].y = rs_obj.op_pset1.at(j).y;
			cloud.points[j].z = rs_obj.op_pset1.at(j).z;
		}
		
		cout<<"\npoint_cloud_publisher.cpp: marker 4\n";
		int idx=0;
		for(int i = 0; i < rs_obj.mat_img1.rows; ++i) //144
			for(int j = 0; j < rs_obj.mat_img1.cols; ++j)//176
			{
				cloud.channels[0].values[idx] = rs_obj.mat_img1.at<float>(i,j) ;
				idx++;
			}

		cout<<"\npoint_cloud_publisher.cpp: marker 5\n";
		cloud_pub.publish(cloud);
		++count;
		r.sleep();
		cout<<"\npoint_cloud_publisher.cpp: marker 6\n";
		//**************************************************
		sensor_msgs::PointCloud cloud2;
		cloud2.header.stamp = ros::Time::now();
		cloud2.header.frame_id = "sensor_frame";

		cloud2.points.resize(rs_obj.op_pset2.size());

		//we'll also add an intensity channel to the cloud
		cloud2.channels.resize(1);
		cloud2.channels[0].name = "intensities";
		cloud2.channels[0].values.resize(144*176);

		//copy data to our point cloud
		for(unsigned int j = 0; j < rs_obj.op_pset2.size(); ++j)
		{
			cloud2.points[j].x = rs_obj.op_pset2.at(j).x;
			cloud2.points[j].y = rs_obj.op_pset2.at(j).y;
			cloud2.points[j].z = rs_obj.op_pset2.at(j).z;
		}
		
		idx=0;
		for(int i = 0; i < rs_obj.mat_img2.rows; ++i) //144
			for(int j = 0; j < rs_obj.mat_img2.cols; ++j)//176
			{
				cloud2.channels[0].values[idx] = rs_obj.mat_img2.at<float>(i,j) ;
				idx++;
			}
